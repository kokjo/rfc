            ...
            let frames: Vec<(usize, u64, [Frame<NormalDshot>; 4]), 4> = match (was_armed, armed) {
                (false, true) => {
                    ...
                    Vec::from_slice(&[
                        (
                            10,
                            0,
                            [Frame::<NormalDshot>::command(Command::SpinDirectionNormal, false); 4],
                        ),
                ...
                (true, true) => {
                    ...
                    Vec::from_slice(&[(
                        ...
                        0,
                        speeds.map(|speed| {
                            Frame::<NormalDshot>::new(speed.min(1999), false).unwrap()
                        }),
                    )])
                    .unwrap()
                }
                (true, false) => {
                    ...
                    Vec::from_slice(&[
                        (
                            10,
                            0,
                            [Frame::<NormalDshot>::command(Command::MotorStop, false); 4],
                        ),
                ...
                (false, false) => {
                    // Disarmed
                    Vec::from_slice(&[(1, 0, [Frame::<NormalDshot>::new(0, false).unwrap(); 4])])
                        .unwrap()
                }
            };
            was_armed = armed;
